...
test_expect_success MULTICORE 'submit second job on rank 0' '
	...
	echo $jobid > jobid.1
'
test_expect_success MULTICORE 'jobid.1: prolog did not start user service' '
	jobid=$(cat jobid.1) &&
	test_debug "cat systemctl-${jobid}.log" &&
	test_must_fail grep "start user@42.service" systemctl-${jobid}.log &&
	test_must_fail grep "stop user@42.service" systemctl-${jobid}.log
'
...
'
test_expect_success 'prolog fails and rolls back service start' '
	jobid=$(submit_as_guest 5m --requires="rank:0" sleep 300) &&
	echo $jobid > jobid.rollback &&
	# Job should fail during prolog (set-property fails)
	test_expect_code 1 flux job wait-event -t 10s $jobid start &&
	test_debug "cat systemctl-${jobid}.log" &&
	# Service should have been started then stopped (rollback)
	grep "start user@42.service" systemctl-${jobid}.log &&
	grep "stop user@42.service" systemctl-${jobid}.log
'
test_expect_success 'verify rollback stop came after start' '
	jobid=$(cat jobid.rollback) &&
	# Extract line numbers to verify stop came after start
	start_line=$(grep -n "start user@42.service" \
		systemctl-${jobid}.log | head -1 | cut -d: -f1) &&
	stop_line=$(grep -n "stop user@42.service" \
		systemctl-${jobid}.log | head -1 | cut -d: -f1) &&
	test -n "$start_line" &&
	test -n "$stop_line" &&
	test "$start_line" -lt "$stop_line"
'
...
'
test_expect_success MULTICORE 'verify only one prolog started the service' '
	# Both jobs should have prolog logs, but only one should start service
	...
	# Count how many times "start user@42.service" appears across both logs
