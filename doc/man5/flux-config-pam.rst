...
2. Count active jobs on the node for this user (excluding the starting job)
3. If this is the first job (count = 0), start ``user@UID.service``
...
Lock files are created with mode 0644 and are never deleted (they persist
to avoid recreating them on each operation).
