...
def debug_log(msg):
    """
    Log debug message to stderr if FLUX_PAM_SCRIPTS_DEBUG is set.

    Args:
        msg: Message to log
    """
    if os.environ.get("FLUX_PAM_SCRIPTS_DEBUG"):
...
class PAMHelper:
    ...
    def check_linger(self, timeout=30):
        ...
        try:
            ...
            raise subprocess.TimeoutExpired(
                [self.loginctl, "show-user"],
                timeout,
                "loginctl timed out checking linger status",
            )
    ...
    def apply_resource_constraints(self, script_type, include_current=False):
        ...
        debug_log(f"{script_type}: computed resource union: {R}")

        # Look up systemd properties for resources
        properties = self.lookup_properties(R)
        debug_log(f"{script_type}: lookup_properties returned {properties}")
        ...
        if properties:
            ...
            debug_log(f"{script_type}: no properties to apply")
    ...
    def _cleanup_user_slice(self):
        ...
        if not orphans:
            debug_log("No orphan processes in slice, cleanup not needed")
            return

        debug_log(f"Found {len(orphans)} orphan scope(s), starting cleanup")
        debug_log(f"Using grace time: {grace_time}s")

        # Step 1: Send SIGTERM
        debug_log("Sending SIGTERM to slice processes")
        self._kill_slice_processes(signal="SIGTERM")

        # Step 2: Wait for grace-time
        debug_log(f"Waiting {grace_time}s for processes to exit")
        if self._wait_for_slice_empty(grace_time):
            debug_log("All processes exited after SIGTERM")
            return

        # Step 3: Send SIGKILL
        remaining = self.check_orphan_processes()
        debug_log(f"{len(remaining)} orphan scope(s) remain, sending SIGKILL")
        self._kill_slice_processes(signal="SIGKILL")

        # Step 4: Wait for grace-time again
        debug_log(f"Waiting {grace_time}s after SIGKILL")
        if self._wait_for_slice_empty(grace_time):
            debug_log("All processes exited after SIGKILL")
            return
    ...
    def user_service_stop(self):
        ...
        cleanup_error = None
        if kill_slice:
            debug_log("kill-user-slice enabled, cleaning up slice")
            try:
